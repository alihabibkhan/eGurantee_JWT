...
def process_upload():
    ...
    try:
        ...
        if len(new_records) > 0:
            ...
            date_columns = ['applicationdate', 'bcc_approval_date', 'business_experiense_(since)', 'experiense_(start_date)'] if file_type == 'pre_disbursement' else ['mis_date', 'booked_on', 'loan_closed_on', 'clo_on', 'dtf', 'dtt']
            ...
            for df_key in new_records:
                ...
                for rec in df.to_dict('records'):
                    ...
                    if file_type == 'pre_disbursement' and str(rec['application_no']) not in ['', 'NaN', 'None']:
                        ...
                        if customer_id not in ['', 'NaN', 'None']:
                            pre_disb_query = f"""
                                SELECT pre_disb_temp_id, KFT_Approved_Loan_Limit
                                FROM tbl_pre_disbursement_temp
                                WHERE "Application_No" = '{customer_id}'
                                ORDER BY pre_disb_temp_id DESC
                                LIMIT 1
                            """
                            pre_disb_result = fetch_records(pre_disb_query)
                            if pre_disb_result:
                                pre_disb_id, kft_approved_limit = pre_disb_result[0]
                                try:
                                    kft_approved_limit = int(float(str(kft_approved_limit or '0')))
                                    disbursed_amount = int(float(str(rec.get('disbursed_amount', '0'))))
                                    if kft_approved_limit != disbursed_amount:
                                        anomalies.append(
                                            f"KFT Approved Amount mismatch: Pre-disbursement = {str(kft_approved_limit)}, Post-disbursement = {str(disbursed_amount)}")
                                except ValueError:
                                    anomalies.append(f"Invalid amount format: KFT_Approved_Loan_Limit = {str(kft_approved_limit)} , disbursed_amount = {str(rec.get('disbursed_amount', ''))}")
                            else:
                                anomalies.append(f"No pre-disbursement record found for customer_id/Application-No = {str(customer_id)}")
                        ...
                        if customer_id not in ['', 'NaN', 'None']:
                            ...
                            pre_disb_result = fetch_records(pre_disb_query)
                            if pre_disb_result:
                                approval_date = pre_disb_result[0]['approved_date']
                                try:
                                    booked_on_date = pd.to_datetime(str(rec.get('booked_on', '')))
                                    approval_date = pd.to_datetime(str(approval_date))
                                    if booked_on_date < approval_date:
                                        anomalies.append(
                                            f"Booked date before approval: booked_on = {str(booked_on_date)}, approval_date = {str(approval_date)}")
                                except ValueError:
                                    anomalies.append(
                                        f"Invalid date format: booked_on = {str(rec.get('booked_on', ''))}, approval_date = {str(approval_date)}")

                        if customer_id not in ['', 'NaN', 'None']:
                            prev_post_query = f"""
                                SELECT branch_name, gender, loan_no, product_code, booked_on, repayment_type,
                                       principal_outstanding, disbursed_amount
                                FROM tbl_post_disbursement
                                WHERE customer_id = '{customer_id}' AND mis_date < '{input_timestamp}'
                                ORDER BY mis_date DESC
                                LIMIT 1
                            """
                            prev_post_result = fetch_records(prev_post_query)
                            if prev_post_result:
                                prev_branch_name, prev_gender, prev_loan_no, prev_product_code, prev_booked_on, \
                                    prev_repayment_type, prev_principal_outstanding, prev_disbursed_amount = \
                                prev_post_result[0]

                                prev_branch_name = str(prev_branch_name or '').strip().lower()
                                prev_gender = str(prev_gender or '').strip().lower()
                                prev_loan_no = str(prev_loan_no or '').strip()
                                prev_product_code = str(prev_product_code or '').strip()
                                prev_booked_on = str(prev_booked_on or '').strip()
                                prev_repayment_type = str(prev_repayment_type or '').strip()
                                prev_principal_outstanding = str(prev_principal_outstanding or '0').strip()
                                prev_disbursed_amount = str(prev_disbursed_amount or '0').strip()

                                curr_branch_name = str(rec.get('branch_name', '')).strip().lower()
                                curr_gender = str(rec.get('gender', '')).strip().lower()
                                curr_loan_no = str(rec.get('loan_no', '')).strip()
                                curr_product_code = str(rec.get('product_code', '')).strip()
                                curr_booked_on = str(rec.get('booked_on', '')).strip()
                                curr_repayment_type = str(rec.get('repayment_type', '')).strip()
                                curr_principal_outstanding = str(rec.get('principal_outstanding', '0')).strip()
                                curr_disbursed_amount = str(rec.get('disbursed_amount', '0')).strip()

                                if prev_branch_name != curr_branch_name:
                                    anomalies.append(
                                        f"Branch Name mismatch: Previous = {str(prev_branch_name)}, Current = {str(curr_branch_name)}")
                                if prev_gender != curr_gender:
                                    anomalies.append(
                                        f"Gender mismatch: Previous = {str(prev_gender)}, Current = {str(curr_gender)}")
                                if prev_loan_no != curr_loan_no:
                                    anomalies.append(
                                        f"Loan Number mismatch: Previous = {str(prev_loan_no)}, Current = {str(curr_loan_no)}")
                                if prev_product_code != curr_product_code:
                                    anomalies.append(
                                        f"Product Code mismatch: Previous = {str(prev_product_code)}, Current = {str(curr_product_code)}")
                                if prev_booked_on != curr_booked_on:
                                    anomalies.append(
                                        f"Booked On mismatch: Previous = {str(prev_booked_on)}, Current = {str(curr_booked_on)}")
                                if prev_repayment_type != curr_repayment_type:
                                    anomalies.append(
                                        f"Repayment Type mismatch: Previous = {str(prev_repayment_type)}, Current = {str(curr_repayment_type)}")

                                try:
                                    curr_outstanding = int(float(curr_principal_outstanding))
                                    prev_outstanding = int(float(prev_principal_outstanding))
                                    curr_disbursed = int(float(curr_disbursed_amount))
                                    if curr_outstanding > prev_outstanding:
                                        anomalies.append(
                                            f"Outstanding amount increased: Previous = {str(prev_outstanding)}, Current = {str(curr_outstanding)}")
                                    if curr_outstanding > curr_disbursed:
                                        anomalies.append(
                                            f"Outstanding exceeds disbursed: Outstanding = {str(curr_outstanding)}, Disbursed = {str(curr_disbursed)}")
                                except ValueError:
                                    anomalies.append(
                                        f"Invalid amount format: principal_outstanding = {str(curr_principal_outstanding)}, disbursed_amount = {str(curr_disbursed_amount)}")

                        if anomalies and customer_id not in ['', 'NaN', 'None']:
                            details = "; ".join(anomalies)
                            insert_anomaly_query = f"""
                                INSERT INTO tbl_post_disbursement_anomalies (pre_disb_id, application_no, details, created_date)
                                VALUES ({pre_disb_id if pre_disb_id else 'NULL'}, '{customer_id}', '{details}', '{datetime.now()}')
                            """
                            execute_command(insert_anomaly_query, is_print=False)
                            print(f"process_upload: Recorded anomalies for application_no='{customer_id}': {details}")
